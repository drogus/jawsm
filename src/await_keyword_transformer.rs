...
impl<'a, 'b> FunctionTranformer<'a, 'b> {
    ...
    fn transform_await_keywords(
        ...
        while let Some(instr) = self.cursor.next() {
            match instr {
                ...
                W::Call(name) if name == "$__async_yield__" || name == "$__async_yield_drop__" => {
                    let function = self.split_at_async_yield(name);
    ...
    fn split_at_async_yield(&mut self, name: String) -> WatFunction {
        ...
        // replace await
        let replacement_instructions = if name == "$__async_yield__" {
            vec![
        ...
        let old_instructions = self
            ...
            .replace_till_the_end_of_function_try_catch_aware(vec![
                ...
                // new_function prepare, first the scope
                W::local_get("$scope"),
                ...
                W::call("$new_function"),
                W::return_call("$resolve_custom_generator_callback"),
            ]);
        ...
        let params_and_locals_extraction: InstructionsList = self
            ...
            .filter(|s| {
                s.name.clone().unwrap() != "$stack"
                    ...
                    && s.name.clone().unwrap() != "$__generator_resolve__"
            })
